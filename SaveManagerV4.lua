...
local SaveManager = {} do
    ...
    SaveManager._layoutHookedLibrary = nil
    SaveManager._layoutPreviousCallback = nil
    SaveManager._layoutCallback = nil
    SaveManager._loadingLayout = false
    SaveManager.LastLoadedConfig = "none"
    SaveManager.LastLoadedConfigSource = "none"
    SaveManager.Parser = {
        Toggle = {
    ...
    function SaveManager:SetLibrary(library)
        if self._layoutHookedLibrary == library then
            self.Library = library
            return
        end
        self:_CancelAutoSave()
        if self._layoutHookedLibrary and self._layoutHookedLibrary.OnLayoutChanged == self._layoutCallback then
            self._layoutHookedLibrary.OnLayoutChanged = self._layoutPreviousCallback
        end
        self.Library = library
        self._autoSaveHooked = {}
        self.LastLoadedConfig = "none"
        self.LastLoadedConfigSource = "none"
        self.LastLoadedConfigLabel = nil
        self._layoutHookedLibrary = library
        local previousCallback = library.OnLayoutChanged
        self._layoutPreviousCallback = previousCallback
        self._layoutCallback = function(...)
            if previousCallback then
                pcall(previousCallback, ...)
            end
            if self.Library == library and not self._loadingLayout and not library.LayoutRestoring and not library.Unloaded then
                self:_QueueAutoSave()
            end
        end
        library.OnLayoutChanged = self._layoutCallback
        library:OnUnload(function()
            if self.Library ~= library then
                return
            end
            self:_CancelAutoSave()
            if library.OnLayoutChanged == self._layoutCallback then
                library.OnLayoutChanged = self._layoutPreviousCallback
            end
            self._layoutHookedLibrary = nil
            self._layoutPreviousCallback = nil
            self._layoutCallback = nil
        end)
        self:RegisterCustomData("LibraryLayout", function()
            return library:GetLayout()
        end, function(data)
            self._loadingLayout = true
            local ran, ok = pcall(library.SetLayout, library, data)
            self._loadingLayout = false
            return ran and ok
        end)
    end
    ...
    function SaveManager:_LoadCustomData(custom)
        if not (custom and next(self.CustomData)) then
            return true
        end
        ...
        for key, handler in self.CustomData do
            if custom[key] ~= nil then
                local Ran, Result = pcall(handler.Load, custom[key])
                if not Ran or Result == false then
                    Error = key == "LibraryLayout" and "failed to load layout" or "failed to load custom data"
                end
            end
        end
        self._loadingLayout = false
    ...
    function SaveManager:LoadAutoloadConfig()
        ...
        if accountConfig then
            local success, err = self:Load(accountConfig, "account")
            if not success then
        ...
        if isfile(autoLoadPath) then
            ...
            local success, err = self:Load(name, "autoload")
            if not success then
    ...
    function SaveManager:_QueueAutoSave()
        ...
        if not self.AutoSave or not library or library.Unloaded then return end

        if self:GetAutoloadConfig() == "none" then return end

        self:_CancelAutoSave()

        local thread
        thread = task.delay(1, function()
            if self._autoSaveThread == thread then
                self._autoSaveThread = nil
            end
            if not self.AutoSave or self.Library ~= library or library.Unloaded then return end
            local name = self:GetAutoloadConfig()
            if name == "none" then return end
            self:Save(name)
        end)
        self._autoSaveThread = thread
    end
    ...
    function SaveManager:_HookElement(idx, element)
        ...
        element.Changed = function(...)
            ...
            if self.Library == library and not library.Unloaded then
                self:_QueueAutoSave()
            end
        end
    end
    ...
    function SaveManager:SetupAutoSave()
        ...
        if not Toggles_mt or not Toggles_mt.__autosave then
            ...
            mt.__newindex = function(t, k, v)
                ...
                if self.Library == library and v and typeof(v) == "table" then
