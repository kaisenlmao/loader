...
function Library:CreateWindow(WindowInfo)
    ...
    do
        ...
        New("ImageLabel", {
            AnchorPoint = Vector2.new(0.5, 0.5),
            BackgroundTransparency = 1,
            Image = "rbxassetid://72530843154458",
            Position = UDim2.fromScale(0.5, 0.5),
        ...
        ToggleButton.MouseEnter:Connect(function()
            if not MainFrame.Visible then
                ToggleStroke.Color = Library.Scheme.AccentColor
                TweenService:Create(ToggleStroke, Library.TweenInfo, { Transparency = 0 }):Play()
            end
        end)

        ToggleButton.MouseLeave:Connect(function()
            if not MainFrame.Visible then
                TweenService:Create(ToggleStroke, Library.TweenInfo, { Transparency = 1 }):Play()
            end
        end)

        MainFrame:GetPropertyChangedSignal("Visible"):Connect(function()
            ToggleStroke.Color = Library.Scheme.AccentColor
            TweenService:Create(ToggleStroke, Library.TweenInfo, {
                Transparency = MainFrame.Visible and 0 or 1,
            }):Play()
        end)
    end
